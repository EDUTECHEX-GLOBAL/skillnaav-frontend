// InstructorManagementedit.js — Light Indigo/Slate Theme (matches InstructureDetailsView)
...
/* ─── Shared style tokens (matching Add form / Details view) ─── */
const inputCls =
...
export default function InstructorManagementedit({ open, item, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      for (let i = 0; i < cleanedSlots.length; i++) {
        ...
        if (!(s.start < s.end)) { prefEndRefs.current[i]?.setCustomValidity("End must be after Start for this slot."); prefEndRefs.current[i]?.reportValidity(); setSubmitting(false); return; }
        if (start && s.start < start) { prefStartRefs.current[i]?.setCustomValidity(`Slot ${i + 1}: Start must be ≥ overall Start (${start}).`); prefStartRefs.current[i]?.reportValidity(); setSubmitting(false); return; }
        if (end && s.end > end) { prefEndRefs.current[i]?.setCustomValidity(`Slot ${i + 1}: End must be ≤ overall End (${end}).`); prefEndRefs.current[i]?.reportValidity(); setSubmitting(false); return; }
      }

      const sorted = cleanedSlots.map((s, idx) => ({ ...s, __idx: idx })).sort((a, b) => a.start.localeCompare(b.start));
      ...
      const payload = {
        ...
        email: (fd.get("email") || "").trim(), phone: fd.get("phone") || "",
        altPhone: fd.get("altPhone") || "", country, state: fd.get("state") || "",
        city: fd.get("city") || "", postalCode: fd.get("postalCode") || "",
        ...
        teachingMode: fd.get("teachingMode") || "Online", bio: fd.get("bio") || "",
        availableDays, availableStart: start || "", availableEnd: end || "",
        timezone: fd.get("timezone") || tz, preferableSlots: cleanedSlots,
        rateType: fd.get("rateType") || "Hourly",
        expectedRate: fd.get("expectedRate") ? Number(fd.get("expectedRate")) : null,
        currency, payoutMethod, payoutIdentifier: fd.get("payoutIdentifier") || "",
        backgroundCheck: fd.get("backgroundCheck") || "Pending", ndaSigned, agreeToTerms,
        assignInternship: fd.get("assignInternship") || "", notes: fd.get("notes") || "",
      };
      ...
      if (!emailForOtp) { alert("Email is required to send OTP."); setSubmitting(false); return; }
  ...
  const doVerifyAndSave = async () => {
    ...
    try {
      ...
      onSaved?.(data);
      setOtpOpen(false); setOtpCode(""); setPendingFormData(null); setPendingId(null);
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" role="dialog" aria-modal="true">
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between px-7 py-4 border-b border-slate-100 bg-white/95 backdrop-blur-sm">
          <div>
            ...
            {(item?.firstName || item?.lastName) && (
              <p className="text-xs text-slate-400 mt-0.5">{item.firstName} {item.lastName}</p>
            )}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-10" noValidate>
            ...
            <FormSection title="Personal & Contact" icon={faEnvelope}>
              <Field label="First Name" required>
                <input name="firstName" required className={inputCls} defaultValue={item?.firstName || ""} placeholder="e.g., Priya" />
              </Field>
              <Field label="Last Name" required>
                <input name="lastName" required className={inputCls} defaultValue={item?.lastName || ""} placeholder="e.g., Sharma" />
              </Field>
              <Field label="Email" required>
                <input type="email" name="email" required className={inputCls} defaultValue={item?.email || ""} placeholder="name@example.com" />
              </Field>
              <Field label="Mobile" required>
                <input name="phone" required className={inputCls} defaultValue={item?.phone || ""} placeholder={phonePlaceholder} />
              </Field>
              <Field label="Alternate Phone">
                <input name="altPhone" className={inputCls} defaultValue={item?.altPhone || ""} placeholder={phonePlaceholder} />
              </Field>
              <Field label="Country" required>
                <select name="country" required className={inputCls} value={country}
                  onChange={(e) => { setCountry(e.target.value); setStateProv(""); setCity(""); setPostalCode(""); setAddress1(""); setAddress2(""); }}>
              ...
              <Field label="City" required>
                <input name="city" required className={inputCls} value={city} onChange={(e) => setCity(e.target.value)} placeholder={cityPlaceholder} />
              </Field>
              <Field label={postalLabel} required>
                <input name="postalCode" required className={inputCls} value={postalCode} onChange={(e) => setPostalCode(e.target.value)} placeholder={country === "Canada" ? "M5V 3L9" : "95113"} />
              </Field>
              <Field label="Address Line 1" required span={2}>
                <input name="address1" required className={inputCls} value={address1} onChange={(e) => setAddress1(e.target.value)} placeholder={address1Placeholder} />
              </Field>
              <Field label="Address Line 2">
                <input name="address2" className={inputCls} value={address2} onChange={(e) => setAddress2(e.target.value)} placeholder="Optional" />
              </Field>
            </FormSection>
            ...
            <FormSection title="Professional & Teaching" icon={faStar}>
              <Field label="Highest Qualification" required>
                <select name="qualification" required className={inputCls} defaultValue={item?.qualification || ""}>
                  <option value="" disabled hidden>Select</option>
                  <option value="Diploma">Diploma</option><option value="Bachelor">Bachelor</option>
                  <option value="Master">Master</option><option value="PhD">PhD</option><option value="Other">Other</option>
                </select>
              </Field>
              <Field label="Years of Experience">
                <input type="number" min="0" step="0.5" name="experienceYears" className={inputCls} defaultValue={item?.experienceYears ?? ""} placeholder="e.g., 3" />
              </Field>
              <Field label="Current / Recent Organization">
                <input name="organization" className={inputCls} defaultValue={item?.organization || ""} placeholder="Company / Institute" />
              </Field>
              <Field label="Teaching Specializations" span={3}>
                <input name="specializations" className={inputCls} defaultValue={initialSpecializations} placeholder="e.g., React, Data Structures, Python (comma-separated)" />
              </Field>
              <Field label="Skills / Technologies" span={3}>
                <input name="skills" className={inputCls} defaultValue={initialSkills} placeholder="e.g., MongoDB, Node.js, AWS (comma-separated)" />
              </Field>
              <Field label="Languages" span={2}>
                <input name="languages" className={inputCls} defaultValue={initialLanguages} placeholder="e.g., English, Spanish (comma-separated)" />
              </Field>
              <Field label="Teaching Mode" required>
                <select name="teachingMode" required className={inputCls} defaultValue={item?.teachingMode || ""}>
                  <option value="" disabled hidden>Select</option>
              ...
              <Field label="Short Bio / Summary" span={3}>
                <textarea name="bio" rows={3} className={textareaCls} defaultValue={item?.bio || ""} placeholder="Brief profile to show on your site" />
              </Field>
            </FormSection>
            ...
            <FormSection title="Availability" icon={faClock}>
                  ...
                  {dayOpts.map((d) => {
                    ...
                    return (
                      <label key={d} className="relative block cursor-pointer">
                        <input type="checkbox" name="availableDays" value={d} defaultChecked={checked}
                          className="peer absolute inset-0 h-full w-full cursor-pointer opacity-0"
                          onChange={(e) => {
                            const formEl = e.currentTarget.form; if (!formEl) return;
              ...
              <Field label="Start Time (24h)" required>
                <input type="time" name="availableStart" className={inputCls} ref={startRef} step="60" required value={availStart}
                  onChange={(e) => {
                    setAvailStart(e.target.value); startRef.current?.setCustomValidity("");
                    (prefStartRefs.current || []).forEach((el) => el?.setCustomValidity(""));
                    (prefEndRefs.current || []).forEach((el) => el?.setCustomValidity(""));
                  }} />
              </Field>
              <Field label="End Time (24h)" required>
                <input type="time" name="availableEnd" className={inputCls} ref={endRef} step="60" required value={availEnd}
                  onChange={(e) => {
                    setAvailEnd(e.target.value); endRef.current?.setCustomValidity("");
                    (prefStartRefs.current || []).forEach((el) => el?.setCustomValidity(""));
                    (prefEndRefs.current || []).forEach((el) => el?.setCustomValidity(""));
                  }} />
              </Field>
              ...
              <div className="md:col-span-3 mt-2">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={addPrefSlot} disabled={!availStart || !availEnd}
                    ...
                    prefSlots.map((s, idx) => (
                      <div key={idx} className="grid grid-cols-1 md:grid-cols-7 gap-3 items-end bg-slate-50 rounded-xl p-3 border border-slate-100">
                        ...
                        <div className="md:col-span-1">
                          <button type="button" onClick={() => removePrefSlot(idx)}
                            className="w-full h-11 rounded-xl border border-rose-200 text-rose-500 text-sm font-medium hover:bg-rose-50 transition-colors">
                            Remove
                          </button>
                        </div>
                      </div>
            ...
            <FormSection title="Compensation / Payout" icon={faDollarSign}>
              <Field label="Rate Type">
                <select name="rateType" className={inputCls} defaultValue={item?.rateType || "Hourly"}>
                  <option>Hourly</option><option>Per Session</option><option>Fixed</option>
                </select>
              </Field>
              <Field label="Expected Rate">
                <input type="number" min="0" step="0.01" name="expectedRate" className={inputCls} defaultValue={item?.expectedRate ?? ""} placeholder="e.g., 1500" />
              </Field>
              ...
              <Field label="Preferred Payout Method">
                <select name="payoutMethod" className={inputCls} value={payoutMethod} onChange={(e) => setPayoutMethod(e.target.value)}>
                  {country === "Canada" ? (
                    <><option>EFT (CA Bank)</option><option>Interac e-Transfer</option><option>PayPal</option></>
                  ) : (
                    <><option>ACH (US Bank)</option><option>Zelle</option><option>PayPal</option></>
                  )}
                </select>
              </Field>
              <Field label="Payout Identifier" span={2}>
                <input name="payoutIdentifier" className={inputCls} defaultValue={item?.payoutIdentifier || ""} placeholder={payIdPlaceholder} />
              </Field>
            </FormSection>
            ...
            <FormSection title="Compliance & Documents" icon={faShieldAlt}>
              <Field label="Resume / CV (upload to replace)">
                <input type="file" name="resume" className={inputCls} accept=".pdf,.doc,.docx" />
              </Field>
              ...
              <Field label="Background Check">
                <select name="backgroundCheck" className={inputCls} defaultValue={item?.backgroundCheck || "Pending"}>
                  <option>Pending</option><option>Cleared</option><option>Not Required</option>
                </select>
              </Field>
              <div className="md:col-span-2 flex items-center gap-6">
                <label className="inline-flex items-center gap-2.5 cursor-pointer group">
                  <input type="checkbox" name="ndaSigned" defaultChecked={!!item?.ndaSigned} className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
                  <span className="text-sm text-slate-600 group-hover:text-slate-800">NDA signed</span>
                </label>
                <label className="inline-flex items-center gap-2.5 cursor-pointer group">
                  <input type="checkbox" name="agreeToTerms" defaultChecked={!!item?.agreeToTerms} className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
                  <span className="text-sm text-slate-600 group-hover:text-slate-800">I confirm all details are accurate</span>
                </label>
              </div>
            </FormSection>
            ...
            <FormSection title="Assignment" icon={faClipboardList}>
              <Field label="Assign to Internship (title or ID)">
                <input name="assignInternship" className={inputCls} defaultValue={item?.assignInternship || ""} placeholder="e.g., MERN Bootcamp – 2025 Summer" />
              </Field>
              <Field label="Notes" span={3}>
                <textarea name="notes" rows={3} className={textareaCls} defaultValue={item?.notes || ""} placeholder="Internal notes" />
              </Field>
            </FormSection>

            {/* Submit */}
            <div className="flex items-center gap-4 pt-2">
              <button type="submit" disabled={submitting || otpOpen}
              ...
              <span className="text-xs text-slate-400">You'll receive an OTP to confirm the update.</span>
            </div>
          </form>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-7 border border-slate-200/60">
            ...
            <input type="text" inputMode="numeric" maxLength={6} placeholder="Enter 6-digit OTP"
              value={otpCode}
              onChange={(e) => { const v = e.target.value.replace(/\D/g, "").slice(0, 6); setOtpCode(v); }}
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              <button type="button" onClick={doVerifyAndSave}
            ...
            <div className="mt-4 text-center">
              <button type="button" className="text-xs text-indigo-600 font-medium hover:underline"
                onClick={async () => {
                  try { await axios.post("/api/instructors/otp/start", { email: otpEmail }, authCfg()); alert("OTP resent."); }
