...
const EmptyState = ({ query, onClear }) => (
  <div className="col-span-full flex flex-col items-center justify-center py-20 text-center">
    ...
    <p className="text-sm text-gray-400 mb-4">
      {query ? 'Try a different keyword or clear the search.' : 'Check back later for new opportunities.'}
    </p>
    {query && (
      <button
        onClick={onClear}
            ...
            <thead className="bg-pink-50 sticky top-0 z-10">
              <tr>
                {['Name', 'Email', 'Applied Date', 'Resume', 'Status'].map(h => (
                  <th key={h} className="text-left px-5 py-3 font-semibold text-gray-600 border-b border-pink-100">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
...
const Pagination = ({ page, totalPages, onPageChange, loading }) => {
  ...
  return (
    <div className="col-span-full flex items-center justify-center gap-2 pt-4 pb-8">
      ...
      {pages[0] > 1 && (
        <>
          <button onClick={() => onPageChange(1)} className="w-9 h-9 rounded-lg border border-gray-200 text-sm text-gray-600 hover:bg-purple-50 hover:border-purple-200 hover:text-purple-600 transition">1</button>
      ...
      {pages[pages.length - 1] < totalPages && (
        <>
          ...
          <button onClick={() => onPageChange(totalPages)} className="w-9 h-9 rounded-lg border border-gray-200 text-sm text-gray-600 hover:bg-purple-50 hover:border-purple-200 hover:text-purple-600 transition">{totalPages}</button>
        </>
      )}
...
const Internships = () => {
  ...
  const filteredCount = internships.length;
    ...
    <div className="min-h-screen bg-gray-50 font-poppins">
        ...
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-9 py-2.5 text-sm rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-transparent bg-gray-50 placeholder-gray-400 transition"
            />
            {searchQuery && (
              <button
                onClick={() => {
            ...
            {loading ? (
              ...
              <>
                {totalCount > 0
                  ...
                  : debouncedQuery ? `No results for "${debouncedQuery}"` : 'No internships found'
                }
      ...
      <div className="p-6 max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        {!loading && !error && internships.length === 0 && (
          <EmptyState query={debouncedQuery} onClear={() => {
            setSearchQuery('');
            clearTimeout(debounceRef.current);
            setPage(1);
            fetchInternships(1, '');
        ...
        {!loading && !error && internships.map((item) => (
          <div key={item._id} className="bg-white shadow-sm hover:shadow-md transition-shadow rounded-2xl p-5 relative border border-gray-100 flex flex-col">
            ...
            {item.skills?.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-1.5">
                {item.skills.slice(0, 5).map((skill, i) => (
                  <span key={i} className="px-2.5 py-0.5 bg-purple-50 text-purple-700 text-xs rounded-full font-medium">{skill}</span>
                ))}
                {item.skills.length > 5 && (
                  <span className="px-2.5 py-0.5 bg-gray-100 text-gray-500 text-xs rounded-full">+{item.skills.length - 5}</span>
                )}
              </div>
            )}
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-xs font-medium">
              <MiniStat icon={<FaLaptopHouse className="text-orange-400" />}   label="Applied"     bg="bg-orange-50 text-orange-700"   onClick={() => openModal('Applied',     item._id)} />
              <MiniStat icon={<AiOutlineStar className="text-green-500" />}    label="Shortlisted" bg="bg-green-50 text-green-700"    onClick={() => openModal('Shortlisted', item._id)} />
              <MiniStat icon={<AiOutlineLike className="text-pink-500" />}     label="Accepted"    bg="bg-pink-50 text-pink-700"      onClick={() => openModal('Accepted',    item._id)} />
              <MiniStat icon={<AiOutlineDislike className="text-indigo-400" />} label="Rejected"   bg="bg-indigo-50 text-indigo-700"  onClick={() => openModal('Rejected',    item._id)} />
            </div>
          </div>
        ))}
