...
export default function AdminFeedbackList({ flow: initialFlowProp } = {}) {
  ...
  }, [page, limit, flowFilter, fromDate, toDate, statusFilter, refreshKey]);
  ...
  // return { key, label, value } or null
  function getRatingKeyAndLabel(item) {
    if (!item) return null;

    // 1) prefer questionMeta entries that look like rating
    if (Array.isArray(item.questionMeta) && item.questionMeta.length) {
      for (const q of item.questionMeta) {
        const id = q.id ?? q.key;
        if (!id) continue;
        const idLower = String(id).toLowerCase();
        if (q.type === "rating" || /(^overall$|overall_|_overall|rating|score|stars)/.test(idLower)) {
          const label = q.label || humanizeKey(String(id));
          const rawVal = item.answers && Object.prototype.hasOwnProperty.call(item.answers, id) ? item.answers[id] : null;
          const numVal = rawVal === null || rawVal === undefined ? null : Number(rawVal);
          return { key: id, label, value: Number.isNaN(numVal) ? rawVal : numVal };
        }
      }
    }

    // 2) fallback: search answers keys for rating-like names
    if (item.answers && typeof item.answers === "object") {
      for (const [k, v] of Object.entries(item.answers)) {
        const kLower = String(k).toLowerCase();
        if (/(^overall$|overall_|_overall|overallpartner|overallschool|rating|score|stars)/.test(kLower)) {
          const label = humanizeKey(k);
          const numVal = v === null || v === undefined ? null : Number(v);
          return { key: k, label, value: Number.isNaN(numVal) ? v : numVal };
        }
      }
    }
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <select 
                ...
                className="border border-gray-300 rounded p-1.5 text-sm focus:ring-1 focus:ring-blue-500 outline-none"
              >
            ...
            <div className="flex items-center gap-2">
              <button 
                ...
                className="px-3 py-1.5 border border-gray-300 rounded text-sm font-medium text-gray-700 hover:bg-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                Previous
              </button>
