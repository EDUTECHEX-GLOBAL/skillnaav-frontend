...
const CertificateApprovals = () => {
  ...
  const [actionModal, setActionModal] = useState({ isOpen: false, type: "", certId: null });
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      toast.success(`Certificate ${actionModal.type.toLowerCase()} successfully`);
  ...
  const filteredCerts = certificates.filter((c) =>
    ...
    c.partnerId?.universityName?.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Certificate Approvals</h1>
          <p className="text-sm text-slate-500 mt-1">Review custom internship certificates uploaded by partners.</p>
        </div>
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all"
          />
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl p-16 text-center shadow-sm border border-slate-100">
          ...
          <p className="text-sm text-slate-500 mt-1">There are no templates matching your criteria.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCerts.map((cert) => (
            <div key={cert._id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-shadow">
              <div 
              ...
              >
                ...
                <div className="absolute top-3 right-3">
                  {cert.status === 'Approved' && <span className="bg-emerald-100 text-emerald-800 text-xs font-bold px-2 py-1 rounded-md shadow-sm">Approved</span>}
                  {cert.status === 'Rejected' && <span className="bg-rose-100 text-rose-800 text-xs font-bold px-2 py-1 rounded-md shadow-sm">Rejected</span>}
                  {cert.status === 'Pending' && <span className="bg-amber-100 text-amber-800 text-xs font-bold px-2 py-1 rounded-md shadow-sm">Pending</span>}
                </div>
              </div>
              
              <div className="p-5 flex-1 flex flex-col">
                <h3 className="font-bold text-slate-800 truncate" title={cert.name}>{cert.name}</h3>
                ...
                <div className="mt-3 flex items-start gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                  ...
                  {cert.partnerId?.logoUrl ? (
                    ...
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-400 to-violet-500 flex items-center justify-center text-white font-bold text-sm shrink-0 border-2 border-white shadow">
                      {cert.partnerId?.name?.charAt(0)?.toUpperCase() || <User size={16} />}
                    </div>
                  )}
                ...
                <p className="text-xs text-slate-400 mt-2">{new Date(cert.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</p>
                
                {cert.status === 'Rejected' && cert.adminRemarks && (
                  <div className="mt-3 bg-rose-50 p-2.5 rounded-lg border border-rose-100">
                    <p className="text-xs font-semibold text-rose-800">Remarks:</p>
                    <p className="text-xs text-rose-600 line-clamp-2">{cert.adminRemarks}</p>
                  </div>
                )}
                ...
                <div className="mt-auto pt-5 flex gap-3">
                  {cert.status !== 'Approved' && (
                    <button
                      onClick={() => setActionModal({ isOpen: true, type: "Approved", certId: cert._id })}
                  ...
                  {cert.status !== 'Rejected' && (
                    <button
                      onClick={() => setActionModal({ isOpen: true, type: "Rejected", certId: cert._id })}
      ...
      <AnimatePresence>
        {actionModal.isOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            ...
            >
              <div className={`p-5 border-b ${actionModal.type === 'Approved' ? 'bg-emerald-50 border-emerald-100' : 'bg-rose-50 border-rose-100'}`}>
                <h3 className={`text-lg font-bold flex items-center gap-2 ${actionModal.type === 'Approved' ? 'text-emerald-800' : 'text-rose-800'}`}>
                  {actionModal.type === 'Approved' ? <CheckCircle2 /> : <XCircle />}
                  {actionModal.type === 'Approved' ? 'Approve Certificate' : 'Reject Certificate'}
                </h3>
              </div>
              <div className="p-5 space-y-4">
                <p className="text-sm text-slate-600">
                  Are you sure you want to mark this certificate as <strong className={actionModal.type === 'Approved' ? 'text-emerald-600' : 'text-rose-600'}>{actionModal.type}</strong>?
                </p>
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    onClick={() => { setActionModal({ isOpen: false, type: "", certId: null }); setAdminRemarks(""); }}
        ...
        {selectedImage && (
          ...
          >
            ...
            >
              ...
              <img src={selectedImage} alt="Certificate Preview" className="w-full h-auto max-h-[85vh] object-contain rounded-lg shadow-2xl bg-white" />
            </motion.div>
          </div>
        )}
      </AnimatePresence>
