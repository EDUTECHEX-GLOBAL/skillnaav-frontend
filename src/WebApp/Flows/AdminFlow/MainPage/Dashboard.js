...
import { FaUsers, FaUserFriends, FaBriefcase, FaDollarSign, FaCalendarAlt, FaChevronDown } from "react-icons/fa";
...
const getPresetRange = (preset) => {
  ...
  switch (preset) {
    case "3m": return { startDate: subtractMonths(2),  endDate };
    case "6m": return { startDate: subtractMonths(5),  endDate };
    case "1y": return { startDate: subtractMonths(11), endDate };
    case "2y": return { startDate: subtractMonths(23), endDate };
    default:   return { startDate: subtractMonths(5),  endDate };
  }
};
 ...
 */
const MONTH_MAP = {
  jan:"01",february:"02",feb:"02",march:"03",mar:"03",april:"04",apr:"04",
  may:"05",june:"06",jun:"06",july:"07",jul:"07",august:"08",aug:"08",
  september:"09",sep:"09",sept:"09",october:"10",oct:"10",
  november:"11",nov:"11",december:"12",dec:"12",january:"01",
};
...
const toYYYYMM = (label) => {
  ...
  if (m1) { const mm = MONTH_MAP[m1[1].toLowerCase()]; return mm ? `${m1[2]}-${mm}` : null; }
  const m2 = s.match(/^(\d{4})[\s-]+([A-Za-z]+)$/);
  if (m2) { const mm = MONTH_MAP[m2[2].toLowerCase()]; return mm ? `${m2[1]}-${mm}` : null; }
  return null;
};
...
const EMPTY_DATA = {
  partnersCount: 0, activeUsersCount: 0, internshipsCount: 0, paymentsCount: 0,
  jobApplications: 0, internshipApprovals: 0, internshipRejections: 0,
  userGrowth: [], jobPostings: [], internshipTypeDistribution: {},
  averageCompensation: {}, partnerApproval: {}, partnerGrowth: [],
  applicationTypeDistribution: {}, totalRevenue: 0, monthlyRevenue: [],
};

// ─── DateRangeFilter ─────────────────────────────────────────────────────────

const DateRangeFilter = ({ preset, customStart, customEnd, onPresetChange, onCustomChange, onApply }) => {
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-xl border border-gray-100 z-50 p-4">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Quick Ranges</p>
              ...
              {PRESETS.map((p) => (
                <button
                  ...
                  className={`text-left px-3 py-2 rounded-lg text-sm transition-colors duration-100 ${
                    preset === p.key ? "bg-indigo-50 text-indigo-700 font-semibold" : "text-gray-600 hover:bg-gray-50"
                  }`}
                >
                  {p.label}
                </button>
            ...
            <div className="flex flex-col gap-2">
              <div>
                ...
                <input
                  type="month" value={customStart} max={customEnd || undefined}
              ...
              <div>
                ...
                <input
                  type="month" value={customEnd} min={customStart || undefined}
...
const Dashboard = () => {
  ...
  const [appliedRange, setAppliedRange] = useState({ startDate: ds, endDate: de });
  ...
  const data = useMemo(() => ({
    ...rawData,
    userGrowth:     filterByRange(rawData.userGrowth,     appliedRange.startDate, appliedRange.endDate),
    jobPostings:    filterByRange(rawData.jobPostings,     appliedRange.startDate, appliedRange.endDate),
    partnerGrowth:  filterByRange(rawData.partnerGrowth,   appliedRange.startDate, appliedRange.endDate),
    monthlyRevenue: filterByRange(rawData.monthlyRevenue,  appliedRange.startDate, appliedRange.endDate),
  ...
  const handleApplyCustom = () => {
    if (customStart && customEnd) setAppliedRange({ startDate: customStart, endDate: customEnd });
  };
  ...
  return (
    <div className="bg-white p-4 sm:p-6 lg:p-8 rounded-lg shadow-md overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6 sm:mb-8">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">
            Showing data from{" "}
            <span className="font-medium text-gray-600">{formatMonthLabel(appliedRange.startDate)}</span>
            {" "}to{" "}
            <span className="font-medium text-gray-600">{formatMonthLabel(appliedRange.endDate)}</span>
            {loading && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
        <Card icon={<FaUserFriends className="h-8 w-8 text-blue-600"   />} title="Partners Enrolled" count={data.partnersCount}    color="bg-blue-100"   />
        <Card icon={<FaUsers       className="h-8 w-8 text-green-600"  />} title="Active Users"      count={data.activeUsersCount} color="bg-green-100"  />
        <Card icon={<FaBriefcase   className="h-8 w-8 text-yellow-600" />} title="Total Internships" count={data.internshipsCount} color="bg-yellow-100" />
        <Card icon={<FaDollarSign  className="h-8 w-8 text-red-600"    />} title="Total Payments"    count={data.paymentsCount}    color="bg-red-100"    />
      </div>
