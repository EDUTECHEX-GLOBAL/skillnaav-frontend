...
import { HiOutlineCurrencyDollar, HiChevronLeft, HiChevronRight } from "react-icons/hi";
        ...
        {totalPages > 1 && (
          <div className="mt-6 flex items-center justify-between">
            ...
            <p className="text-xs text-gray-500">
              Showing{" "}
              <span className="font-medium text-gray-700">{startIndex + 1}–{Math.min(startIndex + PAGE_SIZE, internships.length)}</span>
              {" "}of{" "}
              <span className="font-medium text-gray-700">{internships.length}</span> programs
            </p>
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                ...
                className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-500 hover:bg-gray-50 hover:border-gray-300 disabled:opacity-40 disabled:cursor-not-allowed transition"
              ...
              {getPageNumbers().map((p, i) =>
                p === "..." ? (
                  <span key={`ellipsis-${i}`} className="w-8 h-8 flex items-center justify-center text-xs text-gray-400 select-none">
                    …
                  </span>
                ) : (
                  <button
