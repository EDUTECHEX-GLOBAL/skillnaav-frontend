              ...
              >
                ...
                <div className="relative">
                  ...
                  <div className="absolute top-3 right-3 sm:top-4 sm:right-4">
                    <div className={`inline-flex max-w-full items-center gap-1 px-3 py-1.5 rounded-full text-xs font-medium border ${getStatusColor(user.status)}`}>
                ...
                <div className="pt-12 px-4 sm:px-6 pb-5 sm:pb-6 flex-1 flex flex-col">
                  ...
                  {/* User Info */}
                  <div className="mb-6">
                    <h3 className="font-bold text-lg sm:text-xl text-gray-900 mb-1 truncate">{user.name || "No Name"}</h3>
                    <div className="flex items-center gap-2 text-gray-600 mb-3 min-w-0">
                      <AiOutlineMail size={14} />
                      <span className="text-sm truncate min-w-0">{user.email || "No Email"}</span>
                    </div>

                    {user.universityName && (
                      <div className="flex items-center gap-2 text-gray-700 mb-2 min-w-0">
                        <MdOutlineSchool size={16} />
                        <span className="text-sm truncate min-w-0">{user.universityName}</span>
                      </div>
                    )}
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start gap-4 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-2 min-w-0">
                        <h3 className="font-bold text-lg text-gray-900 break-words">{user.name || "No Name"}</h3>
                        <div className={`inline-flex w-fit items-center gap-1 px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(user.status)}`}>
                          {getStatusIcon(user.status)}
                          <span>{user.status || "Pending"}</span>
                        </div>
                      </div>
                      <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 sm:gap-4 min-w-0">
                        ...
                        {user.universityName && (
                          <div className="flex items-start gap-2 text-gray-600 min-w-0">
                            ...
                            <span className="text-sm break-words">{user.universityName}</span>
                          </div>
                        )}
                        {user.fieldOfStudy && (
                          <div className="flex items-start gap-2 text-gray-600 min-w-0">
                            <AiOutlineBook size={14} />
                            <span className="text-sm break-words">{user.fieldOfStudy}</span>
                          </div>
                        )}
                      </div>
                    </div>
