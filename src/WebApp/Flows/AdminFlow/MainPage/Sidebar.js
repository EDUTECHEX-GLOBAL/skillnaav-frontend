...
// Mini badge shown next to sub-items in the sidebar
const SubBadge = ({ children, color = "green" }) => {
  ...
  return (
    <span
      className={`ml-auto text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${styles[color]}`}
    >
      {children}
    </span>
  );
...
const Sidebar = ({ isOpen, isCompactLayout, onClose, isDesktopOpen = true }) => {
  ...
  const SidebarSection = ({
    ...
    setExpanded,
    items,
    // Optional: per-item badge config: { [key]: { label: string, color: string } }
    itemBadges = {},
  }) => (
    <div>
      <button
          ...
          {items.map((item) => {
            ...
            return (
              ...
              >
                ...
                {badge && (
                  <SubBadge color={badge.color}>{badge.label}</SubBadge>
                )}
              </button>
            );
          })}
    ...
    <>
      ...
      >
        <div className="w-full h-full flex flex-col">
          <div
            className={`flex-1 overflow-y-auto hide-scrollbar pt-4 pb-6 ${
