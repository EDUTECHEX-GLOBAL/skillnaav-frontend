...
import {
  S, globalCss, fmtTime, fmtDate,
  getInitials, getAvatarColor,
  IconBack, IconInfo, IconSend, IconEmptyChat,
...
const Avatar = React.memo(({ title, size = 38 }) => (
  <div style={{
    ...
    background: getAvatarColor(title), display: "flex",
    alignItems: "center", justifyContent: "center",
    flexShrink: 0, fontSize: Math.floor(size * 0.34), fontWeight: 700, color: "#fff",
  }}>
    {getInitials(title)}
  </div>
));
...
const Spinner = React.memo(() => (
  <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", gap: 10 }}>
    <div style={{ width: 22, height: 22, borderRadius: "50%", border: "2.5px solid #E0E7FF", borderTopColor: "#4A6CF7", animation: "spin 0.7s linear infinite" }} />
...
const EmptyState = React.memo(({ icon, title, subtitle }) => (
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", gap: 12, padding: 28 }}>
    <div style={{ width: 64, height: 64, borderRadius: 20, background: "linear-gradient(135deg,#EEF2FF,#E0E7FF)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      {icon}
    </div>
    <p style={{ margin: 0, fontWeight: 700, fontSize: 15.5, color: "#0F172A" }}>{title}</p>
    <p style={{ margin: 0, fontSize: 13, color: "#64748B", textAlign: "center", maxWidth: 280, lineHeight: 1.7 }}>{subtitle}</p>
  </div>
));
...
const TypingIndicator = React.memo(() => (
  <div style={{ display: "flex", justifyContent: "flex-start", marginBottom: 10 }}>
    <div style={{ background: "#fff", border: "1px solid #E9EDF5", borderRadius: "16px 16px 16px 5px", padding: "10px 14px", display: "flex", gap: 4, alignItems: "center", boxShadow: "0 2px 10px rgba(15,23,42,0.05)" }}>
      {[0, 0.15, 0.3].map((delay, i) => (
        <span key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: "#94A3B8", display: "inline-block", animation: "typing 1.2s ease-in-out infinite", animationDelay: `${delay}s` }} />
      ))}
    </div>
  </div>
));
...
const MessagesPager = React.memo(({ currentPage, totalPages, onGoTo, totalCount, isLoading }) => {
  ...
  return (
    <div style={S.msgsPager}>
      <p style={S.pagerText}>Page {currentPage} of {totalPages} · {totalCount} total</p>
      <PaginationBar currentPage={currentPage} totalPages={totalPages} onGoTo={onGoTo} isLoading={isLoading} />
    </div>
  );
});
...
// ─── ChatView ─────────────────────────────────────────────────
const ChatView = ({
  selected, partnerId, messages,
  msgLoading, msgPaging, msgCurrentPage, msgTotalPages, msgTotalCount,
  isTyping, sending, input, inputRef, messagesEndRef,
  showDetailPanel, pendingFiles,
  onBack, onMsgPageChange, onSend, onKeyDown, onInputChange,
  onToggleDetail, onFilesSelected, onRemoveFile,
}) => {
  const fileInputRef = useRef(null);

  const messageList = useMemo(() => messages.map((msg, idx) => {
    const curDate  = fmtDate(msg.timestamp || msg.createdAt);
    const prevDate = idx > 0 ? fmtDate(messages[idx - 1].timestamp || messages[idx - 1].createdAt) : null;
    return (
        ...
        <div style={S.chatCol}>
          ...
          <div style={S.chatHeader}>
            <div style={S.headerRow}>
              <button className="back-btn" onClick={onBack} style={S.backBtn} aria-label="Back">
                {IconBack}
              </button>

              {selected.imgUrl
                ? <img src={selected.imgUrl} alt={selected.title} style={S.headerImg} onError={(e) => { e.target.style.display = "none"; }} />
                : <Avatar title={selected.title} size={38} />
              }

              <div style={S.headerMeta}>
                <h3 style={S.headerTitle}>{selected.title || "Untitled"}</h3>
                <p style={S.headerSub}>{[selected.company, selected.location].filter(Boolean).join(" · ")}</p>
                <div style={S.pillRow}>
                  {selected.intType && (
                  ...
                  {selected.intMode && (
                    <span style={MODE_PILL}>
                      {selected.intMode === "ONLINE" ? "Online" : selected.intMode === "OFFLINE" ? "Offline" : "Hybrid"}
                    </span>
                  )}
                  <span style={ACTIVE_WRAP}><span style={ACTIVE_DOT} />Active</span>
                </div>
              </div>
            </div>
            ...
            <div style={S.headerRight}>
              {msgTotalCount > 0 && (
                <div style={S.msgCount}>
                  <span style={S.msgCountText}>{msgTotalCount} msg{msgTotalCount !== 1 ? "s" : ""}</span>
                </div>
              )}
              <button
                ...
                style={{
                  ...
                  border: showDetailPanel ? "1.5px solid #A5B4FC" : "1px solid #E2E8F0",
          ...
          <div style={S.msgArea} className="scroll-smooth">
            {msgLoading ? <Spinner /> :
              messages?.length ? (
                <>
                  <MessagesPager currentPage={msgCurrentPage} totalPages={msgTotalPages} totalCount={msgTotalCount} onGoTo={onMsgPageChange} isLoading={msgPaging} />
                  {messageList}
                  {isTyping && <TypingIndicator />}
                  <div ref={messagesEndRef} />
                </>
              ) : (
                <EmptyState icon={IconEmptyChat} title="No messages yet" subtitle="Start the conversation with the admin team about this internship." />
              )
            }
          </div>
          ...
          <div style={S.inputBar}>
            <FileAttachmentPreview files={pendingFiles} onRemove={onRemoveFile} />
            <div style={S.inputRow}>
              ...
              <button className="attach-btn" title="Attach files" onClick={() => fileInputRef.current?.click()}
                style={{ width: 42, height: 42, borderRadius: 11, border: "1px solid #E2E8F0", background: "#F8FAFC", cursor: "pointer", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", transition: "background 0.15s" }}>
                {IconAttach}
              </button>
              ...
              <button onClick={onSend} disabled={!canSend} style={{ ...S.sendBtn, opacity: !canSend ? 0.5 : 1 }} aria-label="Send">
                {sending
                  ? <div style={{ width: 18, height: 18, borderRadius: "50%", border: "2px solid rgba(255,255,255,0.4)", borderTopColor: "#fff", animation: "spin 0.7s linear infinite" }} />
                  : IconSend
                }
              </button>
            </div>
