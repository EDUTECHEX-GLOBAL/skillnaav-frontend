...
import axios from "../../../../api/axiosInstance";import Modal from "react-modal";
...
const exportCSV = (payments, internshipTitle) => {
  ...
  const headers = ["Student Name", "Student Email", "Amount", "Currency", "Status", "PayPal Order ID", "PayPal Payment ID", "Date"];
...
const StatusBadge = ({ status }) => {
  const cfg = {
    COMPLETED: { cls: "bg-green-100 text-green-700 border-green-200",   icon: faCircleCheck,  label: "Completed" },
    CREATED:   { cls: "bg-yellow-100 text-yellow-700 border-yellow-200", icon: faClock,        label: "Pending"   },
    FAILED:    { cls: "bg-red-100 text-red-600 border-red-200",          icon: faCircleXmark,  label: "Failed"    },
    CANCELLED: { cls: "bg-gray-100 text-gray-500 border-gray-200",       icon: faCircleXmark,  label: "Cancelled" },
    APPROVED:  { cls: "bg-blue-100 text-blue-700 border-blue-200",       icon: faClock,        label: "Approved"  },
  }[status] || { cls: "bg-gray-100 text-gray-500 border-gray-200", icon: faClock, label: status };
  ...
  >
    ...
    {internship.imgUrl ? (
      ...
      <div className="w-16 h-16 rounded-lg bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-100 flex items-center justify-center flex-shrink-0">
        <FontAwesomeIcon icon={faBriefcase} className="text-indigo-300 text-xl" />
      </div>
    )}

    {/* Info */}
    <div className="flex-1 min-w-0">
      <h3 className="text-sm font-bold text-gray-900 truncate mb-0.5">{internship.jobTitle}</h3>
      <p className="text-xs text-indigo-500 font-medium mb-2 flex items-center gap-1">
        <FontAwesomeIcon icon={faBuilding} className="text-[9px] text-indigo-300" />
      ...
      {internship.qualifications?.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-3">
          {internship.qualifications.slice(0, 4).map((q, i) => (
            <span key={i} className="text-[11px] bg-gray-100 text-gray-500 px-2.5 py-0.5 rounded-full">
              {q}
            </span>
          ))}
...
const InternshipPayments = () => {
  ...
  const modalTotalPages = Math.max(1, Math.ceil(filteredPayments.length / MODAL_PER_PAGE));
  const pageData = filteredPayments.slice((page - 1) * MODAL_PER_PAGE, page * MODAL_PER_PAGE);

  // Modal summary stats — always from ALL payments (not filtered)
  const completedPayments = payments.filter((p) => p.status === "COMPLETED");
  const totalReceived = completedPayments.reduce((s, p) => s + (p.amount || 0), 0);
  const currency = completedPayments[0]?.currency || payments[0]?.currency || "USD";

  // ── Render ────────────────────────────────────────────────────────────────
  return (
    <div className="min-h-screen bg-slate-50 px-6 py-8" style={{ fontFamily: "'Poppins', sans-serif" }}>
      <div className="max-w-5xl mx-auto">

        {/* ── Page Header ── */}
        <div className="flex items-start gap-3 mb-6">
          <div className="w-10 h-10 rounded-xl bg-indigo-600 flex items-center justify-center flex-shrink-0">
            <FontAwesomeIcon icon={faFileInvoiceDollar} className="text-white" />
          </div>
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5" style={{ fontFamily: "'Poppins', sans-serif" }}>
              Select a paid internship to view PayPal payments received from students
            </p>
          </div>
        </div>
        ...
        <div className="relative mb-4">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white"
            style={{ fontFamily: "'Poppins', sans-serif" }}
          />
          {listSearch && (
            <button
        ...
        {!loadingInternships && !internshipError && (
          <p className="text-xs text-gray-400 mb-4 flex items-center gap-1.5"
            ...
            {listSearch && <> matching <em className="text-gray-600">"{listSearch}"</em></>}
          </p>
        )}

        {/* ── Error ── */}
        {internshipError && (
          <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-sm text-red-600 mb-4"
            style={{ fontFamily: "'Poppins', sans-serif" }}>
        ...
        {!loadingInternships && !internshipError && internships.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-gray-400">
            <div className="w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center mb-4">
              <FontAwesomeIcon icon={faFileInvoiceDollar} className="text-2xl text-gray-300" />
            </div>
            <p className="text-sm font-medium text-gray-500" style={{ fontFamily: "'Poppins', sans-serif" }}>
              {!debouncedSearch
                ? "No approved paid internships found."
                : `No internships match "${debouncedSearch}".`}
            </p>
            <p className="text-xs text-gray-300 mt-1" style={{ fontFamily: "'Poppins', sans-serif" }}>
              {!debouncedSearch
      ...
      >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-6 py-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
            ...
            />
            <button
              onClick={() => exportCSV(filteredPayments, selectedInternship?.jobTitle)}
              disabled={filteredPayments.length === 0 || loadingPayments}
              className="flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white rounded-xl text-sm font-semibold transition"
              style={{ fontFamily: "'Poppins', sans-serif" }}
            >
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-8 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300"
                style={{ fontFamily: "'Poppins', sans-serif" }}
              />
              {search && (
                <button
          ...
          {!loadingPayments && !paymentError && payments.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-gray-400">
              <div className="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center mb-3">
                <FontAwesomeIcon icon={faMoneyBillWave} className="text-xl text-gray-300" />
              </div>
          ...
          {!loadingPayments && !paymentError && payments.length > 0 && filteredPayments.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12 text-gray-400">
              <FontAwesomeIcon icon={faMagnifyingGlass} className="text-2xl text-gray-200 mb-3" />
              <p className="text-sm" style={{ fontFamily: "'Poppins', sans-serif" }}>
                    ...
                    {[
                      ...
                      <th key={label}
                        ...
                        <span className="flex items-center gap-1.5">
                          <FontAwesomeIcon icon={icon} className="text-gray-300 text-[9px]" />
                          {label}
                        </span>
                      </th>
                    ))}
                    ...
                    <tr key={p._id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-5 py-3">
                        <p className="text-xs font-semibold text-gray-800 flex items-center gap-1.5">
                          <FontAwesomeIcon icon={faUser} className="text-indigo-300 text-[9px]" />
                      ...
                      <td className="px-5 py-3 font-mono text-xs text-gray-500 max-w-[140px]">
                        <span className="flex items-center gap-1 truncate">
                          <FontAwesomeIcon icon={faHashtag} className="text-gray-300 text-[9px] flex-shrink-0" />
                          {p.paypalPaymentId
