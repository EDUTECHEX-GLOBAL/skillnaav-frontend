...
const ChatInterface = () => {
  ...
  const { list: internships, loading: intLoading, searching: intSearching, currentPage: intCurrentPage, totalPages: intTotalPages, totalCount: intTotalCount, searchDraft, committedQuery } = intState;

  // ── Selected chat ──
  const [showChat,  setShowChat]  = useState(false);
  const [selected,  setSelected]  = useState({ id: null, title: "", company: "", imgUrl: "", intType: "", intMode: "", location: "" });
  ...
  const { list: messages, loading: msgLoading, paging: msgPaging, currentPage: msgCurrentPage, totalPages: msgTotalPages, totalCount: msgTotalCount } = msgState;
  ...
  const fetchInternships = useCallback(async (page, query, isFirst = false) => {
    ...
    try {
      ...
      intDispatch({ type: "FETCH_SUCCESS", data: res.data.data || [], totalPages: res.data.totalPages || 1, totalCount: res.data.total || 0, query });
    } catch (err) {
  ...
  useEffect(() => { if (!partnerId) return; fetchInternships(1, "", true); isIntInitialRef.current = false; }, [partnerId, fetchInternships]);
  useEffect(() => { if (isIntInitialRef.current) return; fetchInternships(intCurrentPage, committedQuery); }, [intCurrentPage, committedQuery, fetchInternships]);
  useEffect(() => { if (isIntInitialRef.current) return; intDispatch({ type: "SET_PAGE", page: 1 }); fetchInternships(1, debouncedSearch); }, [debouncedSearch, fetchInternships]);

  const handleSearchSubmit = useCallback(() => { intDispatch({ type: "SET_PAGE", page: 1 }); fetchInternships(1, searchDraft); }, [searchDraft, fetchInternships]);
  ...
  const handleViewDetailsFromList = useCallback(async (internship) => {
  ...
  const handleCloseDetailPanel = useCallback(() => { setShowDetailPanel(false); setSelectedInternship(null); }, []);
  ...
  const fetchMessages = useCallback(async (page, internshipId, isFirst = false) => {
    ...
    try {
      const res = await axios.get(`/api/chats/partner/${partnerId}/internship/${internshipId}`, { params: { page, limit: MESSAGES_PER_PAGE } });
  ...
  useEffect(() => { if (isMsgInitialRef.current) return; fetchMessages(msgCurrentPage, currentInternRef.current); }, [msgCurrentPage, fetchMessages]);
  ...
  const handleInternshipClick = useCallback((id, title, extra = {}) => {
    setSelected({ id, title, company: extra.companyName || "", imgUrl: extra.imgUrl || "", intType: extra.internshipType || "", intMode: extra.internshipMode || "", location: extra.location || "" });
  ...
  const handleMsgPageChange = useCallback((page) => msgDispatch({ type: "SET_PAGE", page }), []);
  const handleFilesSelected  = useCallback((newFiles) => setPendingFiles((prev) => [...prev, ...newFiles]), []);
  const handleRemoveFile     = useCallback((idx) => setPendingFiles((prev) => prev.filter((_, i) => i !== idx)), []);

  // ─── Send ──────────────────────────────────────────────────
  const handleSend = useCallback(async () => {
    const hasText  = input.trim();
    const hasFiles = pendingFiles.length > 0;
    if ((!hasText && !hasFiles) || !partnerId || !selected.id || sending) return;

    const text        = hasText ? input.trim() : "";
    const filesToSend = [...pendingFiles];

    setInput(""); setPendingFiles([]); setSending(true); setIsTyping(true);

    const optimistic = {
      _id: `temp-${Date.now()}`, message: text, sender: partnerId,
      internship: selected.id, timestamp: new Date().toISOString(), createdAt: new Date().toISOString(),
      files: filesToSend.map((f) => ({ originalName: f.name, mimeType: f.type, size: f.size, url: f.type.startsWith("image/") ? URL.createObjectURL(f) : null })),
    };
      ...
      if (filesToSend.length > 0) {
        ...
        for (let i = 0; i < uploadedFiles.length; i++) {
          ...
          res = await axios.post("/api/chats/send", {
            internshipId: selected.id, senderId: partnerId,
            message: i === 0 ? text : "",
            fileUrl: fileData.fileUrl, fileName: fileData.fileName, fileType: fileData.fileType, fileSize: fileData.fileSize,
          });
        }
      } else {
        res = await axios.post("/api/chats/send", { internshipId: selected.id, senderId: partnerId, message: text });
      }
      msgDispatch({ type: "REPLACE_LAST", msg: res.data });
    } catch (err) {
      console.error("sendMessage:", err);
      msgDispatch({ type: "REMOVE", id: optimistic._id });
      setInput(text); setPendingFiles(filesToSend);
    } finally {
      setSending(false);
    ...
    return (
      <>
        ...
        <div style={S.noSession}>
          <div style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: 22, padding: "30px 24px", width: "min(92%, 420px)", boxShadow: "0 20px 44px rgba(15,23,42,0.06)", textAlign: "center" }}>
            <div style={S.noSessionIcon}>{IconNoSession}</div>
            <h2 style={{ margin: 0, fontSize: 21, fontWeight: 800, color: "#0F172A" }}>Session not found</h2>
            <p style={{ margin: "10px 0 0", color: "#64748B", lineHeight: 1.7, fontSize: 13.5 }}>
              We couldn't detect your partner session. Please log in again and reopen the messaging page.
            </p>
          </div>
        </div>
      </>
        ...
        {showChat ? (
          ...
          <div style={{ ...S.rowFull }}>
            ...
            {!mobile && (
              <div style={{ ...S.sidebarWrap, width: desktop ? 300 : 260 }}>
                <InternshipListView
                  internships={internships} intLoading={intLoading} intSearching={intSearching}
                  intCurrentPage={intCurrentPage} intTotalPages={intTotalPages} intTotalCount={intTotalCount}
                  searchDraft={searchDraft} committedQuery={committedQuery} selectedId={selected.id}
                  onSearchDraftChange={(v) => intDispatch({ type: "SET_DRAFT", value: v })}
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <ChatView
                selected={selected} partnerId={partnerId} messages={messages}
                msgLoading={msgLoading} msgPaging={msgPaging}
                msgCurrentPage={msgCurrentPage} msgTotalPages={msgTotalPages} msgTotalCount={msgTotalCount}
                isTyping={isTyping} sending={sending} input={input}
                inputRef={inputRef} messagesEndRef={messagesEndRef}
                showDetailPanel={showDetailPanel} pendingFiles={pendingFiles}
                onBack={handleBackToList} onMsgPageChange={handleMsgPageChange}
                onSend={handleSend} onKeyDown={handleKeyDown} onInputChange={setInput}
                onToggleDetail={handleToggleDetailPanel}
                onFilesSelected={handleFilesSelected} onRemoveFile={handleRemoveFile}
              />
            </div>
          ...
          <div style={{ display: "flex", height: "100%", width: "100%", overflow: "hidden" }}>
            ...
            <div style={{
              ...
              flexShrink: 0, height: "100%", background: "#fff",
              borderRight: showDetailPanel && !mobile ? "1px solid #EAECF4" : "none",
              ...
              <InternshipListView
                internships={internships} intLoading={intLoading} intSearching={intSearching}
                intCurrentPage={intCurrentPage} intTotalPages={intTotalPages} intTotalCount={intTotalCount}
                searchDraft={searchDraft} committedQuery={committedQuery} selectedId={null}
                onSearchDraftChange={(v) => intDispatch({ type: "SET_DRAFT", value: v })}
                onSearchSubmit={handleSearchSubmit}
                onPageChange={(page) => intDispatch({ type: "SET_PAGE", page })}
                onInternshipClick={handleInternshipClick}
            ...
            {showDetailPanel && !mobile && (
              <div style={{ flex: 1, minWidth: 0, height: "100%", overflow: "hidden", display: "flex", background: "#fff" }}>
                <InternshipDetailPanel
                  open={showDetailPanel}
                  item={selectedInternship}
                  loading={detailLoading}
