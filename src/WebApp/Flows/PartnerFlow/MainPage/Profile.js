...
const ProfileForm = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const userInfo = (JSON.parse(localStorage.getItem("partnerInfo")) || JSON.parse(localStorage.getItem("userInfo")));
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent("partnerUpdated", { detail: updatedUserInfo }));
    ...
    <div className="min-h-screen mt-12 flex justify-center bg-gray-50 font-poppins pb-12">
      <div className="w-full max-w-4xl bg-white p-6 rounded-lg shadow-lg">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <h2 className="text-3xl font-bold text-gray-800">Partner Profile</h2>
            <p className="text-gray-500">
              {isEditing ? "Edit your organizational details" : "View your organizational details"}
            </p>
          </div>
          ...
          {isSectionOpen && (
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-2 pb-6 border-b border-gray-100">
                <label className="text-sm font-medium text-gray-700">Logo / Profile Image</label>
                <div className="flex items-center gap-6">
                  <div className="w-24 h-24 bg-gray-100 rounded-full flex items-center justify-center overflow-hidden border-2 border-gray-300">
                    {(isEditing ? tempUser.profileImagePreview : user.profileImage) ? (
                      <img 
                        className="w-full h-full object-cover" 
                        src={isEditing ? tempUser.profileImagePreview : user.profileImage} 
                        alt="Profile" 
                      />
                    ) : (
                  ...
                  {isEditing && (
                    <div>
                      ...
                      />
                      <p className="text-xs text-gray-500 mt-2">Upload a new image (JPEG, PNG, JPG)</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-gray-700">Full Name / Representative Name</label>
                  {isEditing ? (
                    <input
                      type="text"
                      name="name"
                ...
                <div className="space-y-2">
                  ...
                  <div className="border border-gray-200 bg-gray-100 text-gray-500 px-3 py-2 rounded-lg min-h-[42px] flex items-center cursor-not-allowed">
                    {user.email}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-gray-700">Organization / University Name</label>
                  {isEditing ? (
                    <input
                      type="text"
                      name="universityName"
                ...
                {/* Institution ID */}
                <div className="space-y-2">
                  <label className="text-sm font-medium text-gray-700">Institution ID</label>
                  {isEditing ? (
                    <input
                      type="text"
                      name="institutionId"
