...
// FIX 1: Frontend plan definitions — price/duration here is display-only.
...
export default function PartnerPremiumPage() {
  ...
  // FIX 2: selectedPlanType stored separately — only planType sent to server
  ...
  // FIX 4: Expiry warning (within 3 days)
  ...
  // FIX 4: Check expiry warning on mount
  useEffect(() => {
  ...
  useEffect(() => {
    const renderButtons = async () => {
      ...
      const buttons = window.paypal.Buttons({
        ...
        createOrder: async () => {
          try {
            // FIX 1: Only send planType — server derives amount and duration
        ...
        onApprove: async (data) => {
          ...
          try {
            // FIX 1: Only send orderID and planType — server derives everything else
            ...
            if (verify.success) {
              ...
              // FIX 3: Clear history so next open fetches fresh
              setPaymentHistory([]);
    ...
    renderButtons();
  }, [selectedIndex, selectedPlanType, sdkReady]);
  ...
  const selectPlan = (plan, idx) => {
    ...
    // FIX 1: Only store planType — no amount passed anywhere
