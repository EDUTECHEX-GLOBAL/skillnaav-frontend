...
const PartnerForgotPasswordModal = ({ isOpen, onClose }) => {
    ...
    const handleResendOtp = async () => {
        ...
        setIsLoading(true);
        try {
            await axios.post('/api/partners/request-password-reset', { email });
 ...
 const handleVerifyOtp = async (e) => {
  ...
  otp: finalOtp,
   ...
   const handleResetPassword = async (e) => {
    ...
    setIsLoading(true);
    try {
        const response = await axios.post('/api/partners/verify-otp-reset-password', {
            email,
            otp: otp || finalOtp,
            newPassword,
        });
    ...
    const stepMeta = {
        1: { title: 'Forgot password?', subtitle: "Enter your partner email to receive a code", icon: <MailIcon /> },
            ...
            <div style={{
                ...
                <div style={{ padding: '28px 28px 0' }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px', marginTop: '16px', alignItems: 'center' }}>
                        {[1, 2, 3].map(s => (
                            <div key={s} className="pfp-step-dot" style={{
                                backgroundColor: s === currentStep ? accent : s < currentStep ? accentMid : '#e5e7eb',
                ...
                <div style={{ padding: '20px 28px 28px', animation: 'fadeSlideIn 0.3s ease' }}>
                    ...
                    {step === 1 && (
                        <form onSubmit={handleRequestOtp} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                            ...
                            <button type="submit" disabled={isLoading} className="pfp-btn-primary" style={btnBase}>
                    ...
                    {step === 2 && !otpVerified && (
                        <form onSubmit={handleVerifyOtp} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                {timeLeft > 0 ? (
                                    <p style={{ margin: 0, fontSize: '13px', color: '#6b7280' }}>
                                        Code expires in{' '}
                                        <span style={{ color: timeLeft < 30 ? '#ef4444' : accent, fontWeight: '600', fontVariantNumeric: 'tabular-nums' }}>
                                            {formatTime(timeLeft)}
                                        </span>
                                    </p>
                                ) : (
                            ...
                            <button type="submit" disabled={isLoading || otp.length < 6} className="pfp-btn-primary" style={btnBase}>
                    ...
                    {message.includes("successfully updated") && (
                        <div style={{
                            ...
                            <a href="/partner/login" style={{
                                ...
                                color: accent, textDecoration: 'none', padding: '6px 16px',
                                border: `1.5px solid ${accentLight}`, borderRadius: '8px',
                            }}>
                                Go to partner login →
                            </a>
                        </div>
