...
function UploadOverlay({ modal, onClose }) {
  ...
  const STEP_ORDER = ["parsing", "validating", "generating", "emailing", "done"];
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:8 }}>
            <div style={{
              ...
              {isDone ? (
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                  <path d="M4 10l4.5 4.5 8-9" stroke="#059669" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              ) : (
            ...
            <div>
              <div style={{ fontSize:17, fontWeight:700, color:"#0F172A" }}>
                {isDone ? "All Credentials Generated!" : "Generating Credentials…"}
              </div>
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontSize:11, fontWeight:600, color:"#94A3B8", letterSpacing:"0.08em", marginBottom:6 }}>
              ...
              <div style={{ display:"flex", alignItems:"baseline", gap:6 }}>
                ...
                <span style={{ fontSize:24, color:"#CBD5E1", fontWeight:300 }}>/</span>
                <span style={{ fontSize:24, fontWeight:600, color:"#94A3B8" }}>{total}</span>
              </div>

              {/* Current student name */}
              {!isDone && currentName && (
                <div style={{
                  fontSize:12, color:"#6366F1", marginTop:7,
                  fontStyle:"italic",
            ...
            <svg width="70" height="70" viewBox="0 0 70 70">
              <circle cx="35" cy="35" r="29" fill="none" stroke="#E2E8F0" strokeWidth="6"/>
              <circle
                cx="35" cy="35" r="29"
                fill="none"
                ...
                style={{ transition:"stroke-dashoffset 0.38s ease, stroke 0.38s ease" }}
              />
              ...
              return (
                <div key={s.key} style={{ display:"flex", alignItems:"center", gap:10 }}>
                  <div style={{
                    ...
                    {done ? (
                      <svg width="11" height="11" viewBox="0 0 11 11" fill="none">
                        <path d="M2 5.5l2.5 2.5 4.5-5" stroke="#059669" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                    ) : active ? (
                      <div style={{
                  ...
                  <span style={{
                    ...
                    {/* Show live "X of Y" inside generating step */}
          ...
          {!isDone && (
            <div style={{
              ...
              <span style={{ fontSize:12, color:"#92400E", lineHeight:1.5 }}>
                🔒 Full page including sidebar is locked until credential generation completes.
              </span>
            </div>
          )}
          ...
          >
            {isDone ? "✓  Done — Close" : `Generating…  ${generated} / ${total}`}
          </button>
        </div>
      </div>
    </>
...
export default function Dashboard() {
  ...
  const [uploadModal, setUploadModal] = useState({
    show: false, generated: 0, total: 0,
    phase: "idle",   // "parsing"|"validating"|"generating"|"emailing"|"done"
    currentName: "",
  });
  ...
  // Simulates per-student progress locally while waiting for the server.
  // ─── REPLACE with real SSE events from your backend for true live counts ───
    ...
    Papa.parse(selectedFile, {
      ...
      complete: async (results) => {
        const requiredHeaders = [
          "Full Name", "Email Address", "School Name",
          "Grade", "Stream/Curriculum", "Field of Internship",
        ];
        const uploadedHeaders = Object.keys(results.data[0] || {});
        const isValid = requiredHeaders.every((h) => uploadedHeaders.includes(h));
        ...
        // Phase 1 — parsing
        setUploadModal({ show:true, generated:0, total, phase:"parsing", currentName:"" });
        await delay(700);
        ...
        // Phase 3 — generating (upload + live counter run in parallel)
        setUploadModal((p) => ({ ...p, phase:"generating" }));
        const formData = new FormData();
        formData.append("csvFile", selectedFile);

        try {
          // Run API call + local progress animation simultaneously
          // If your backend supports SSE, swap simulatePerStudentProgress() with:
          //
          //   const src = new EventSource(`/api/school-admin/upload-stream?token=${token}`);
          //   src.onmessage = (e) => {
          //     const { generated, name } = JSON.parse(e.data);
          //     setUploadModal(p => ({ ...p, generated, currentName: name }));
          //   };
          ...
          const [res] = await Promise.all([
            ...
            simulatePerStudentProgress(students), // ← remove if using SSE
          ]);
          ...
          setUploadModal({ show:false, generated:0, total:0, phase:"idle", currentName:"" });
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen font-poppins">
      ...
      {/* 🔒 Blur overlay — position:fixed covers sidebar + full viewport */}
          ...
          >
            ...
            {stat.type === "upload" ? (
              <div className="flex flex-col gap-2">
                ...
                {progress && (
                  <div className="mt-1">
                    <div className="flex justify-between text-xs text-white mb-1">
                      ...
                      <span className="font-bold">{progress.generated}/{progress.total}</span>
                    </div>
                ...
                <a
                  href="/student_template.csv" download
