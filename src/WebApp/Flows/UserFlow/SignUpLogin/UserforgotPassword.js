...
const ForgotPasswordModal = ({ isOpen, onClose }) => {
    ...
    const stepTitles = {
        1: { title: 'Forgot password?', subtitle: "Enter your email and we'll send you a code", icon: <MailIcon /> },
        2: { title: 'Check your email', subtitle: `We sent a 6-digit code to ${email}`, icon: <ShieldIcon /> },
        3: { title: 'New password', subtitle: 'Choose a strong password for your account', icon: <LockIcon /> },
    };

    const currentStep = otpVerified ? 3 : step;
    const { title, subtitle, icon } = stepTitles[currentStep];
        ...
        <div style={{
            ...
            <div style={{
                ...
                <style>{`
                    ...
                    .fp-input:focus { border-color: #6366f1 !important; box-shadow: 0 0 0 3px rgba(99,102,241,0.1) !important; background: #fff !important; }
                    .fp-btn-primary { background: linear-gradient(135deg, #6366f1 0%, #4f46e5 100%); color: #fff; }
                    .fp-btn-primary:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 6px 20px rgba(99,102,241,0.35); }
                    .fp-btn-primary:active:not(:disabled) { transform: translateY(0); }
                    .fp-btn-primary:disabled { opacity: 0.65; }
                    .fp-otp-box:focus { border-color: #6366f1 !important; box-shadow: 0 0 0 3px rgba(99,102,241,0.12) !important; background: #fff !important; }
                    .fp-otp-box.filled { border-color: #6366f1; background: #eef2ff; color: #4338ca; }
                    .fp-eye-btn:hover { color: #6366f1; }
                ...
                <div style={{ padding: '28px 28px 0' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '4px' }}>
                            ...
                            <div>
                                <h2 style={{ margin: 0, fontSize: '18px', fontWeight: '700', color: '#111827', lineHeight: 1.2 }}>{title}</h2>
                                <p style={{ margin: '3px 0 0', fontSize: '12.5px', color: '#6b7280', lineHeight: 1.4 }}>{subtitle}</p>
                            </div>
                        </div>
                        <button onClick={onClose} style={{
                            background: 'none', border: 'none', cursor: 'pointer',
                            color: '#9ca3af', fontSize: '20px', lineHeight: 1, padding: '2px 4px',
                    ...
                    <div style={{ display: 'flex', gap: '6px', marginTop: '16px', alignItems: 'center' }}>
                        {[1, 2, 3].map(s => (
                            <div key={s} className="fp-step-dot" style={{
                                backgroundColor: s === currentStep ? '#6366f1' : s < currentStep ? '#a5b4fc' : '#e5e7eb',
                ...
                {/* Body */}
                <div style={{ padding: '20px 28px 28px', animation: 'fadeSlideIn 0.3s ease' }}>
                    ...
                    {error && (
                        <div style={{
                            ...
                            marginBottom: '16px', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '8px',
                        }}>
                    ...
                    {message && !message.includes("successfully updated") && (
                        <div style={{
                            ...
                            color: '#16a34a', padding: '10px 14px', borderRadius: '10px',
                            marginBottom: '16px', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '8px',
                        }}>
                                    ...
                                    {otpDigits.map((digit, i) => (
                                        <input
                                            ...
                                            style={{
                                                ...
                                                backgroundColor: digit ? '#eef2ff' : '#fafafa',
                                                color: digit ? '#4338ca' : '#111827',
                                                caretColor: '#6366f1',
                                            }}
                                        />
                                    ))}
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                {timeLeft > 0 ? (
                                    <p style={{ margin: 0, fontSize: '13px', color: '#6b7280' }}>
                                        Code expires in{' '}
                                        <span style={{ color: timeLeft < 30 ? '#ef4444' : '#6366f1', fontWeight: '600', fontVariantNumeric: 'tabular-nums' }}>
                                            {formatTime(timeLeft)}
                                        </span>
                                    </p>
                                ) : (
                    ...
                    {otpVerified && (
                        <form onSubmit={handleResetPassword} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                                ...
                                <div style={{ position: 'relative' }}>
                                    ...
                                    <button
                                        ...
                                        style={{
                                            ...
                                            background: 'none', border: 'none', cursor: 'pointer',
                                            color: '#9ca3af', padding: '2px', display: 'flex', transition: 'color 0.15s',
                                        }}
                            ...
                            <div>
                                ...
                                {confirmPassword && confirmPassword === newPassword && (
                                    <p style={{ fontSize: '12px', color: '#16a34a', margin: '4px 0 0', display: 'flex', alignItems: 'center', gap: '4px' }}>
                            ...
                            <div style={{ background: '#f9fafb', borderRadius: '10px', padding: '10px 12px' }}>
                                <p style={{ margin: '0 0 6px', fontSize: '12px', fontWeight: '600', color: '#6b7280' }}>Password must include:</p>
                                {[
                                    ...
                                    <div key={label} style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '3px' }}>
                                        <div style={{
                                            ...
                                            <span style={{ fontSize: '9px', color: test ? '#16a34a' : '#9ca3af' }}>{test ? '✓' : '·'}</span>
                                        </div>
                                        <span style={{ fontSize: '12px', color: test ? '#374151' : '#9ca3af', transition: 'color 0.2s' }}>{label}</span>
                                    </div>
                                ))}
                            </div>
                    ...
                    {message.includes("successfully updated") && (
                        <div style={{
                            ...
                            <a href="/user/login" style={{
                                ...
                                color: '#6366f1', textDecoration: 'none', padding: '6px 16px',
                                border: '1.5px solid #e0e7ff', borderRadius: '8px',
