...
const StudentAssessment = () => {
  ...
  /* =================================
     VIOLATION HANDLER
  ================================= */
  const handleViolation = useCallback(async (violation, count) => {
  ...
  useEffect(() => {
    timingPatternRef.current = timingPattern;
  }, [timingPattern]);
  ...
  const handleSubmit = useCallback(async (forced = false) => {
    ...
    try {
      ...
      const timingPatternArray = Object.entries(timingPatternRef.current).map(
      ...
      if (err.response?.status === 410) {
        ...
        const latestEvaluation = err.response.data?.evaluation;
        if (latestEvaluation) {
  ...
  }, [assessmentId, evaluateAssessment, exitFullscreen, stopProctoring]);
  ...
  }, [exitFullscreen, mode, stopProctoring]);
