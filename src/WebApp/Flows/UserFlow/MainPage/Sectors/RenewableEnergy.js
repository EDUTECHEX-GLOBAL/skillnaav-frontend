...
const RenewableEnergy = () => {
  ...
  // Fetch internships for "Renewable Energy" sector when component mounts
  useEffect(() => {
    const fetchSectorInternships = async () => {
      try {
        setLoading(true); // Show loading message
        const res = await axios.get("/api/interns/approved", {
          params: { isPremium: false, sector: "renewable-energy" }, // Filtering by sector
        });
  ...
  return (
    <div className="p-4 font-poppins">
      ...
      <p className="text-gray-600 mb-4">
        Explore the latest opportunities in the field of Renewable Energy & Grid Innovation.
      </p>
