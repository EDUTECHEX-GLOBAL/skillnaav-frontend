...
const MaterialScience = () => {
  ...
  // Fetch internships for "Material Science" sector when component mounts
  useEffect(() => {
    const fetchSectorInternships = async () => {
      try {
        setLoading(true); // Show loading message
        const res = await axios.get("/api/interns/approved", {
          params: { isPremium: false, sector: "materials-science" }, // Filtering by sector
        });
  ...
  return (
    <div className="p-4 font-poppins">
      {/* Page Title */}
      <h1 className="text-2xl font-bold mb-2">Advanced Materials Science Internships ({internships.length})</h1>

      {/* Short description */}
      <p className="text-gray-600 mb-4">
        Explore the latest opportunities in the field of Advanced Materials Science.
      </p>
