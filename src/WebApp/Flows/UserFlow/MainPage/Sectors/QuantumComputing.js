...
const QuantumComputing = () => {
  ...
  // Fetch internships for "Quantum Computing" sector when component mounts
  useEffect(() => {
    const fetchSectorInternships = async () => {
      try {
        setLoading(true); // Show loading message
        const res = await axios.get("/api/interns/approved", {
          params: { isPremium: false, sector: "quantum-computing" }, // Filtering by sector
        });
  ...
  return (
    <div className="p-4 font-poppins">
      {/* Page Title */}
      <h1 className="text-2xl font-bold mb-2">Quantum Computing Internships ({internships.length})</h1>

      {/* Short description */}
      <p className="text-gray-600 mb-4">
        Explore the latest opportunities in the field of Quantum Computing and Next-Gen Computing.
      </p>
