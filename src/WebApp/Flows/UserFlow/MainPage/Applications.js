...
const resolveApplicationStatus = (pipeline) => {
  ...
  // ✅ L3 (highest priority)
  if (pipeline.l3?.status === "passed") return "Interview Cleared"; // 👈 ADD THIS
  ...
  if (pipeline.l2?.status === "rejected") return "Applied";
  ...
  const statusColors = {
    ...
    "Interview Invite Sent": "bg-green-100 text-green-800",
      "Interview Cleared": "bg-emerald-100 text-emerald-700",
