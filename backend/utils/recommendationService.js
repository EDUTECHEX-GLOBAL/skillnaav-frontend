...
async function getPersonalizedRecommendations(studentId, limit = 6) {
  ...
  // Enrich with past applications
  const inferred = await inferFromRecentApplications(studentId);

  // --- If no past internships, default level = Basic ---
  ...
  const candidates = await InternshipPosting.find(baseQuery)
    .select("jobTitle companyName location internshipMode qualifications compensationDetails stipendMin stipendMax applicationDeadline jobDescription sector classification")
    .limit(100)
    .lean();
  ...
  const scored = candidates.map(j => ({
      ...j,
      _matchScore: scoreJob(j, signals, student)
  }))
  .sort((a, b) => b._matchScore - a._matchScore);

  // Fallback: recent basic+intermediate internships if nothing strong
