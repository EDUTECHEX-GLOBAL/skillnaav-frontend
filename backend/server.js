...
// Partner cron (we will start this manually after DB connects)
...
app.post("/analyze-skills", async (req, res) => {
  try {
    const resp = await axios.post(`${process.env.FASTAPI_BASE_URL}/analyze-skills`, req.body, { timeout: 30000 });
    res.json(resp.data);
  } catch (error) {
    console.error("Error from FastAPI:", error.response?.data || error.message);
    res.status(502).json({ error: "Upstream service error" });
...
io.on("connection", (socket) => {
  ...
  // Partner-specific room so cron can emit updates to that partner
...
// ------------------- Connect DB, start cron(s), and listen -------------------
connectDB()
  .then(() => {
    console.log("✔ MongoDB connected");

    // Start Partner cron (pass io so it can emit realtime events)
    startPartnerCron({ io, cronExpr: "0 * * * *", timezone: "UTC" }); // runs hourly

    // Note: user cron (checkpremiumExipiration) auto-schedules on import so we don't need to start it here.
